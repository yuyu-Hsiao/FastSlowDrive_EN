        ...
        while True:
            ...
            for event in events:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_PAUSE:
                    paused = True
                    world.hud.notification("Paused", seconds=1.0)
                if event.type == pygame.KEYDOWN and event.key == pygame.K_e:
                    paused = False
                    world.hud.notification("Start", seconds=1.0)
                else:
                    filtered_events.append(event)
                if event.type == pygame.QUIT:
                    return

            # 如果處於暫停狀態，就降低迴圈頻率並跳過更新
            if paused:
            ...
            # 將過濾後的事件傳給 parse_events
            if controller.parse_events(client, world, clock, args.sync, events=filtered_events):
                return
            world.tick(clock)
